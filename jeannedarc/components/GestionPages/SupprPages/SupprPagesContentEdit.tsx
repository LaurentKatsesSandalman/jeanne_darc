...
export function SupprPagesContentEdit({ btn }: SupprPagesContentEditProps) {
    ...
    async function handleDelete() {
        ...
        if (!deletePageResult.success) {
            console.error("Échec de la requête:", deletePageResult);
            if ("errors" in deletePageResult) {
                setError(
        ...
        if (buttons) {
            for (const button of buttons) {
                if (button.position > position) {
                    const updatePositionResult =
                        await updateContenuHeaderBtnAction(
                            button.id_contenu_headerbtn,
